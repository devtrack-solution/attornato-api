...
describe('[APPLICATION] - ListMachineService', () => {
  ...
  it('should return paginated machines', async () => {
    ...
    const select = MachineTestBuilder.getSelect()
    const searchFields = MachineTestBuilder.getSearchFields()
    const order = MachineTestBuilder.getOrder()
    ...
    expect(machineRepository.findAllByCriteria).toHaveBeenCalledWith(criteria, order, select, searchFields, relations)
