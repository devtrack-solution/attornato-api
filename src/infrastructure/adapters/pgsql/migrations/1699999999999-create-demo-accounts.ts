...
export default class CreateDemoAccounts1699999999999 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const user of users) {
      ...
      await queryRunner.query(
        `INSERT INTO "account_persons" ("id", "name", "birthday") VALUES ($1, $2, $3)`,
        [accountPersonId, user.name, new Date('1990-01-01')],
      )

      // 2. Credential
      await queryRunner.query(
        `INSERT INTO "credentials" ("id", "username", "password_hash", "request_change_password") VALUES ($1, $2, $3, $4)`,
        [credentialId, user.email, passwordHash, false],
      )

      // 3. Account
      await queryRunner.query(
        `INSERT INTO "accounts" ("id", "accountPersonId", "credentialId") VALUES ($1, $2, $3)`,
        [accountId, accountPersonId, credentialId],
      )
      ...
      for (const roleName of ['ADMINISTRATOR', 'COLLABORATOR']) {
        ...
        if (roleId) {
          await queryRunner.query(
            `INSERT INTO "credential_role" ("id_credential", "id_role") VALUES ($1, $2) ON CONFLICT DO NOTHING`,
            [credentialId, roleId],
          )
        }
      }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    const emails = [
      'laercio@devtrack.com.br',
      'wilton@devtrack.com.br',
      'anderson@devtrack.com.br',
    ]
