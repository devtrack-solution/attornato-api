...
export class ListToSelectMachineGroupService implements ListToSelectMachineGroupInboundPort {
  ...
  async execute(criteria: Criteria.FindBy): Promise<Partial<MachineGroupType.Output[]>> {
    const select: string[] = ['id', 'groupName', 'slug', 'groupCode']
    const searchFields: string[] = ['groupName']
    const order = { groupName: 'ASC' }
    let machineGroups = await this.machineGroupRepository.findForSelectByCriteria(criteria, order, select, searchFields)
