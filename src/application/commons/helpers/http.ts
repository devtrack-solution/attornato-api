import { ServerError } from '@/application/commons/errors'
...
export const badRequest = (error: BadRequestException): HttpResponse<ResponseError> => ({
  statusCode: HttpStatus.BAD_REQUEST,
  message: error.message ?? 'Falha na requisição!',
})
export const conflict = (error: ConflictException): HttpResponse<ResponseError> => {
  const response = error.getResponse() as { message: string; data?: any }
  return {
    statusCode: HttpStatus.CONFLICT,
    data: new ResponseError(HttpStatus.CONFLICT, error.message ?? 'Conflito na requisição!', response.data ?? response),
  }
}

export const unauthorized = (error: UnauthorizedException): HttpResponse<ResponseError> => ({
  statusCode: HttpStatus.UNAUTHORIZED,
  message: error.message ?? 'Acesso não autorizado!',
})

export const notfound = (message: NotFoundException): HttpResponse<ResponseError> => ({
...
export const serverError = (error: unknown): HttpResponse<ResponseError> => ({
  ...
  message: new ServerError(error instanceof Error ? error : 'Error inesperado').message,
})
